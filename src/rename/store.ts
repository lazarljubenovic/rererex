...
function handleReducerFile (file: SourceFile, store: string, newName: string) {
  file.getImportDeclarationOrThrow(`./${ casing.param(newName) }/reducer`)
    .renameDefaultImport(casing.camel(newName))
...
function handleStateFile (file: SourceFile, store: string, newName: string) {
  file.getImportDeclarationOrThrow(`./${ casing.param(newName) }/state`)
    .renameDefaultImport(casing.pascal(newName))
...
function handleIndexFile (file: SourceFile, store: string, newName: string) {
  file.getImportDeclarationOrThrow(`./${ casing.param(newName) }`).remove()
